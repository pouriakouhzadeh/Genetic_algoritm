...
from model_tester_H1 import ModelTester
from FEATURESELECTION import ParallelFeatureSelection
...
with open('initial_parameters_H1.txt', 'r') as file:
    content = file.read()

numbers_list = content.split('\n')

count_init, count_page_creator_init, count_k_means_init, count_feature_init, i_init, counter_init = map(int, numbers_list[:6])
#-----------------------------------------------------------
...
for count in range (count_init,0 ,-10000) :
    ...
    for count_page_creator in range(count_page_creator_init, 10):
        ...
        data, target = PageCreator().create(data, target, count_page_creator)
        ...
        for count_k_means in range(count_k_means_init, 7) :
            ...
            for count_feature in range (count_feature_init, 200, 2) :
                ...
                feature_selector = ParallelFeatureSelection()
                for i in range(i_init, count_k_means) :
                    ...
                    print(f"|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||")
                    with open('initial_parameters_H1.txt', 'w') as file:
                        file.write(f"{count}\n{count_page_creator}\n{count_k_means}\n{count_feature}\n{i}\n{counter}")
