...
def win_los(Data_EURUSD_for_train_input,Data_EURUSD_for_train_target,Data_EURUSD_for_answer):
    ...
    position, Answer = DesisionClass().Desision(Data_EURUSD_for_answer ,model ,page = 2 ,feature = 30 ,QTY = 1000 ,depth = 2, Thereshhold = 65)
    if position == "BUY":
...
for i in range (22000,24900):
   ...
   if Time.hour > 17 and Time.hour != 0 :
        ...
        try :
            ...
            logging.info(f"BT65 : wins = {wins} ----- loses = {loses} ---- ACC = {wins/(wins+loses)}") 
        except:
            print(f"wins = {wins} ----- loses = {loses}")    
            logging.info(f"BT65 : wins = {wins} ----- loses = {loses}")     
